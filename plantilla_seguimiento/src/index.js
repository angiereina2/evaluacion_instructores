...
function filevent(){
  ventfile = new BrowserWindow({
      width: 600,
      height:600,
      title: 'Carga Masiva Datos',
      webPreferences: {
          preload: path.join(__dirname, 'controlador/cargamasiva.js'),
      }
  })
  ventfile.loadFile('src/vista/cargamasiva.html')  
  ventfile.setMenu(null);
};
...
function validatelogin(obj){
    ...
    connection.query(sql, [email, password, rol], (error, results)=>{
        if(error){
            console.log(error);
        }
        if(results.length>0 && rol=='administrador'){
            ventanaindex()
            mainWindow.show()
            ventlogin.close()
        }
        else if(results.length>0 && rol=='evaluador'){
            ventanainvitado()
            invitadoWindow.show()
            ventlogin.close()
        }else{
...
const nuevoMenu=[      
    ...
    {
        label: 'Registro y Consulta',
        submenu: [
          {
            label: 'Carga Masiva Datos',
            accelerator: process.platform=='darwin' ? 'command+D': 'Ctrl+', 
            click(){
                filevent();
            }
        },
